...
class ServiceMeteo {
  String _descriptionDepuisCode(int code) {
    ...
    if ([1, 2, 3].contains(code)) return 'meteo_partiellement_nuageux'.tr();
  ...
  String _conseilDepuisMeteo({
    ...
    if (d.contains('orage') || d.contains('عاصفة')) {
    ...
    if (d.contains('brouillard') || d.contains('ضباب')) {
      return 'meteo_conseil_brouillard'.tr();
    }

    if (vent >= 35) return 'meteo_conseil_vent'.tr();
    if (temperature >= 32) return 'meteo_conseil_chaud'.tr();
    if (temperature <= 10) return 'meteo_conseil_froid'.tr();
  ...
  int margeTrajetSelonMeteo(MeteoActuelle meteo) {
    ...
    if (d.contains('orage') || d.contains('عاصفة')) return 30;
    ...
    if (d.contains('brouillard') || d.contains('ضباب')) return 20;
...
Future<Position?> obtenirPositionActuelle() async {
  ...
  Future<({double latitude, double longitude, String nom})?> _chercherVille(
    ...
    final url = Uri.parse(
      ...
      '?name=${Uri.encodeComponent(ville)}'
      '&count=1'
      '&language=fr'
      '&format=json',
    );
  ...
  Future<MeteoActuelle> obtenirMeteoActuelle({
    String ville = 'Sousse',
  }) async {
    try {
     final gps = await obtenirPositionActuelle();

final position = gps != null
    ? (
    ...
    : await _chercherVille(ville);
      ...
      final ressenti =
          ((current['apparent_temperature'] ?? temperature) as num).round();
      final humidite =
          ((current['relative_humidity_2m'] ?? 0) as num).round();
  ...
  Future<List<PrevisionJour>> obtenirPrevisions({
    ...
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Erreur API prévisions');
      }
